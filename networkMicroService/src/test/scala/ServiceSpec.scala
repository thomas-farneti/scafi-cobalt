...
import it.unibo.scafi.cobalt.services.networkService.{NetworkRepository, NetworkService, Router}
...
class ServiceSpec extends WordSpec with Matchers with ScalatestRouteTest with SprayJsonSupport{
  ...
  "The Network service" should {
    ...
    "Return a list of devices" in {
      Get("/neighbors/1a3t") ~> new Router(new NetworkService(repository)).routes ~> check {
